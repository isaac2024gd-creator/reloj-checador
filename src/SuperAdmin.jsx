...
const PLANES = {
  ...
  multi_sucursal: { nombre: "Multi-sucursal", variable: true },
};

/* ------------------------------------------------------------
   Multi-sucursal — precio base + variable (ver costos-y-precios-
   mercado.md y alta-clientes-y-roles.md, confirmado 18 sep 2026).

   - Base $1,799: hasta 3 sucursales y 60 empleados incluidos.
   - Cada sucursal extra YA TRAE 20 empleados incluidos:
       4ª-8ª sucursal: $599 c/u · 9ª en adelante: $449 c/u.
   - Empleados de más, SIN abrir sucursal nueva, en bloques:
       bloque de 5: primeros 8 bloques (=40 empleados) a $99,
       de ahí en adelante $79 por bloque.
       bloque de 50 (para saltos grandes de una sola vez): $699,
       siempre más barato por empleado que ir en bloques de 5 —
       el cálculo los usa primero.
   ------------------------------------------------------------ */
const MS_BASE = {
  precio: 1799,
  sucursalesIncluidas: 3,
  empleadosIncluidos: 60,
  empleadosPorSucursalExtra: 20,
};
const MS_SUCURSAL_EXTRA_TOPE_TRAMO1 = 8; // 4a a 8a sucursal
const MS_SUCURSAL_EXTRA_PRECIO_TRAMO1 = 599;
const MS_SUCURSAL_EXTRA_PRECIO_TRAMO2 = 449; // 9a en adelante
const MS_BLOQUE5_TOPE_TRAMO1 = 8; // primeros 8 bloques de 5 (40 empleados)
const MS_BLOQUE5_PRECIO_TRAMO1 = 99;
const MS_BLOQUE5_PRECIO_TRAMO2 = 79; // bloque 9 de 5 en adelante
const MS_BLOQUE50_PRECIO = 699;

function calcularMultiSucursal(sucursalesInput, empleadosInput) {
  const sucursales = Math.max(MS_BASE.sucursalesIncluidas, Math.round(Number(sucursalesInput) || MS_BASE.sucursalesIncluidas));
  const empleados = Math.max(MS_BASE.empleadosIncluidos, Math.round(Number(empleadosInput) || MS_BASE.empleadosIncluidos));

  const sucursalesExtra = sucursales - MS_BASE.sucursalesIncluidas;

  let costoSucursales = 0;
  for (let i = 1; i <= sucursalesExtra; i++) {
    const numeroDeSucursal = MS_BASE.sucursalesIncluidas + i;
    costoSucursales += numeroDeSucursal <= MS_SUCURSAL_EXTRA_TOPE_TRAMO1 ? MS_SUCURSAL_EXTRA_PRECIO_TRAMO1 : MS_SUCURSAL_EXTRA_PRECIO_TRAMO2;
  }

  // Empleados que ya vienen cubiertos por la base + lo que trae cada sucursal extra.
  const empleadosIncluidos = MS_BASE.empleadosIncluidos + sucursalesExtra * MS_BASE.empleadosPorSucursalExtra;
  const empleadosPorCubrir = Math.max(0, empleados - empleadosIncluidos);

  // Cubrir primero con bloques de 50 (más baratos por empleado), el resto en bloques de 5.
  const bloques50 = Math.floor(empleadosPorCubrir / 50);
  const restanteParaBloques5 = empleadosPorCubrir - bloques50 * 50;
  const bloques5 = Math.ceil(restanteParaBloques5 / 5);

  const costoBloques50 = bloques50 * MS_BLOQUE50_PRECIO;
  let costoBloques5 = 0;
  for (let i = 1; i <= bloques5; i++) {
    costoBloques5 += i <= MS_BLOQUE5_TOPE_TRAMO1 ? MS_BLOQUE5_PRECIO_TRAMO1 : MS_BLOQUE5_PRECIO_TRAMO2;
  }

  const total = MS_BASE.precio + costoSucursales + costoBloques50 + costoBloques5;
...
function FormularioCliente({ pin, onCreated, onCreatedKeepOpen, onError, onToast }) {
  ...
  const [sucursalesMS, setSucursalesMS] = useState(MS_BASE.sucursalesIncluidas);
  const [empleadosMS, setEmpleadosMS] = useState(MS_BASE.empleadosIncluidos);
  ...
  const calcMS = plan === "multi_sucursal" ? calcularMultiSucursal(sucursalesMS, empleadosMS) : null;

  // Precio y límites efectivos a guardar, según el plan.
  const precioFinal = plan === "multi_sucursal" ? calcMS.total : info.precio;
  const limiteEmpleadosFinal = plan === "multi_sucursal" ? calcMS.empleados : info.limite_empleados;
  const limiteSucursalesFinal = plan === "multi_sucursal" ? calcMS.sucursales : info.limite_sucursales;

  function limpiarParaOtro() {
    setNombreNegocio("");
    setPlan("basico");
    setSucursalesMS(MS_BASE.sucursalesIncluidas);
    setEmpleadosMS(MS_BASE.empleadosIncluidos);
    setShowDueno(false);
    setNombreDueno("");
    setEmailDueno("");
    setTelefonoDueno("");
  ...
  async function crear() {
    ...
    try {
      const { data, error } = await supabase.rpc("crear_cliente_reloj", {
        ...
        p_plan: plan,
        p_precio_mensual: precioFinal,
        p_limite_empleados: limiteEmpleadosFinal,
        p_limite_sucursales: limiteSucursalesFinal,
    ...
    return (
      <div className="rounded-sm p-4 mb-4" style={{ background: paper }}>
        ...
        <div className="text-xs rounded-sm p-3 mb-3" style={{ background: "#fff", border: `1px solid ${ink}22`, color: ink }}>
          ...
          {resumen.plan === "multi_sucursal" && (
            <div style={{ color: ink + "88" }}>
              {resumen.limite_sucursales} sucursales · {resumen.limite_empleados} empleados
            </div>
          )}
  ...
  return (
    <div className="rounded-sm p-4 mb-4" style={{ background: paper }}>
        ...
        {Object.entries(PLANES).map(([key, p]) => (
          ...
          >
            ...
            <span className="text-xs" style={{ color: ink + "88" }}>
              {p.variable
                ? `${money(MS_BASE.precio)} base · desde ${MS_BASE.sucursalesIncluidas} sucursales / ${MS_BASE.empleadosIncluidos} empleados`
                : (
                  <>
      ...
      {plan === "multi_sucursal" && (
        <div className="rounded-sm p-3 mb-3" style={{ background: "#fff", border: `1px solid ${ink}22` }}>
          <div className="text-xs font-bold uppercase mb-2" style={{ color: ink, letterSpacing: "0.06em" }}>
            Tamaño del negocio (para calcular el precio)
          </div>

          <div className="flex gap-2 mb-3">
            <div className="flex-1">
              <label className="text-xs font-bold block mb-1" style={{ color: ink + "99" }}>Sucursales totales</label>
              <input
                type="number"
                min={MS_BASE.sucursalesIncluidas}
                value={sucursalesMS}
                onChange={(e) => setSucursalesMS(e.target.value)}
                className="w-full px-3 py-2 rounded-sm text-sm outline-none"
                style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
              />
            </div>
            <div className="flex-1">
              <label className="text-xs font-bold block mb-1" style={{ color: ink + "99" }}>Empleados totales</label>
              <input
                type="number"
                min={MS_BASE.empleadosIncluidos}
                value={empleadosMS}
                onChange={(e) => setEmpleadosMS(e.target.value)}
                className="w-full px-3 py-2 rounded-sm text-sm outline-none"
                style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
              />
            </div>
          ...
          <div className="text-xs flex flex-col gap-1" style={{ color: ink + "aa" }}>
            <div className="flex justify-between"><span>Base ({MS_BASE.sucursalesIncluidas} sucursales / {MS_BASE.empleadosIncluidos} empleados)</span><span>{money(MS_BASE.precio)}</span></div>
            {calcMS.sucursalesExtra > 0 && (
              <div className="flex justify-between">
                <span>{calcMS.sucursalesExtra} sucursal{calcMS.sucursalesExtra > 1 ? "es" : ""} extra (con {calcMS.sucursalesExtra * MS_BASE.empleadosPorSucursalExtra} empleados incluidos)</span>
                <span>{money(calcMS.costoSucursales)}</span>
              </div>
            )}
            {calcMS.bloques50 > 0 && (
              <div className="flex justify-between">
                <span>{calcMS.bloques50} bloque{calcMS.bloques50 > 1 ? "s" : ""} de 50 empleados</span>
                <span>{money(calcMS.costoBloques50)}</span>
              </div>
            )}
            {calcMS.bloques5 > 0 && (
              <div className="flex justify-between">
                <span>{calcMS.bloques5} bloque{calcMS.bloques5 > 1 ? "s" : ""} de 5 empleados</span>
                <span>{money(calcMS.costoBloques5)}</span>
              </div>
            )}
            <div className="flex justify-between font-bold pt-1 mt-1" style={{ color: ink, borderTop: `1px solid ${ink}22` }}>
              <span>Total</span>
              <span>{money(calcMS.total)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="text-xs rounded-sm p-3 mb-4" style={{ background: "#fff", border: `1px solid ${ink}22`, color: ink + "aa" }}>
        Resumen: <b>{nombreNegocio || "(nombre del negocio)"}</b> — plan <b>{info.nombre}</b>, {money(precioFinal)}
        {plan === "multi_sucursal" ? ` (${limiteSucursalesFinal} sucursales / ${limiteEmpleadosFinal} empleados)` : ""}.
      </div>
