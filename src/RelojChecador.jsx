...
// true cuando la sesión activa entró con "magic link" (correo) en vez de nombre+PIN.
// La cambian las funciones de más abajo (enviarEnlaceAcceso/miSesionPorToken) y lockNow().
let sesionViaCorreo = false;

async function leerDatoProtegido(clave, usuarioNombre, pin) {
  if (sesionViaCorreo) {
    const { data, error } = await supabase.rpc("dato_protegido_leer_reloj_auth", {
      p_acceso_token: ACCESO_TOKEN,
      p_clave: clave,
    });
    if (error) throw error;
...
async function guardarDatoProtegido(clave, valor, usuarioNombre, pin) {
  if (sesionViaCorreo) {
    const { data, error } = await supabase.rpc("dato_protegido_guardar_reloj_auth", {
      p_acceso_token: ACCESO_TOKEN,
      p_clave: clave,
      p_valor: valor,
    });
...
/* ------------------------------------------------------------
   Login por cliente (correo / magic link) — opcional, convive con el
   nombre+clave de siempre. El Dueño/Gerente que ya se agregó un correo
   (ver usuario_asignar_mi_correo_reloj) puede entrar tocando un enlace que
   le llega por correo, sin volver a escribir su clave.
   ------------------------------------------------------------ */
async function enviarEnlaceAcceso(email) {
...
async function miSesionPorToken() {
  const { data, error } = await supabase.rpc("mi_sesion_reloj_token", { p_acceso_token: ACCESO_TOKEN });
  if (error) throw error;
  return data && data.length ? data[0] : null;
}

async function asignarMiCorreo(usuarioNombre, pin, correo) {
  const { data, error } = await supabase.rpc("usuario_asignar_mi_correo_reloj", {
    p_acceso_token: ACCESO_TOKEN,
    p_usuario_nombre: usuarioNombre,
    p_pin: pin,
    p_correo: correo,
...
// ---------- hora del servidor (NO confiar en el reloj del dispositivo) ----------
// Un empleado podría adelantar o atrasar el reloj de su celular para falsear la hora
// de entrada/salida. Para evitarlo, en vez de usar `new Date()` (hora del dispositivo)
// en la foto y en el registro de checado, pedimos la hora real al servidor: hacemos un
// HEAD al propio backend de Supabase y leemos el header HTTP "Date" de la respuesta,
// que pone el servidor (no el navegador) — así aunque el celular tenga mal la fecha,
// la marca de agua y el registro guardado usan la hora verdadera.
// Si no hay conexión o falla la petición, NO se bloquea el checado (un corte de internet
// en una hora crítica no debe frenar los cálculos automáticos de puntualidad/nómina):
// se cae de vuelta a la hora del dispositivo, pero marcada como `verified: false`. Quien
// llama (tomar foto / confirmar checado) debe guardar esa bandera en el registro, para
// que se vea un aviso en Bitácora y el negocio decida qué hacer con esos casos puntuales.
async function getServerNow() {
  try {
    ...
    if (dateHeader) {
      ...
      // se compensa la mitad del viaje de ida y vuelta de la petición, para acercar
      // más el resultado al instante real en que se toma la foto o se confirma el checado.
      if (!Number.isNaN(serverMs)) return { date: new Date(serverMs + roundTripMs / 2), verified: true };
    }
  } catch (err) {
    console.error("No se pudo sincronizar la hora con el servidor, se usa la hora del dispositivo:", err);
  }
  return { date: new Date(), verified: false };
}

// ---------- marca de agua con fecha y hora (solo fotos de checado, no el logo) ----------
function drawTimestampWatermark(ctx, w, h, stampDate, verified = true) {
  const baseStamp = (stampDate || new Date()).toLocaleString("es-MX", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  const stamp = verified ? baseStamp : `${baseStamp} · SIN VERIFICAR`;
  ...
  // franja ámbar en vez de negra cuando la hora NO se pudo confirmar contra el servidor
  // (se usó la del dispositivo) — así se nota a simple vista en la foto misma.
  ctx.fillStyle = verified ? "rgba(0,0,0,0.6)" : "rgba(193,68,45,0.85)";
...
function compressImage(file, maxWidth = 480, quality = 0.6, { watermark = false, stampDate, verified = true } = {}) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        if (watermark) drawTimestampWatermark(ctx, w, h, stampDate, verified);
...
function computePayrollTotals(draft, worked, bonoUnits, propinaMonto, employee, feriadosMonto) {
  ...
  const consumo = draft.enableConsumo
    ...
    : 0;
  const bruto = base + bono + propina + vacaciones + feriados + horasExtra;
  ...
  return { base, bono, propina, vacaciones, feriados, horasExtra, descuentos, consumo, bruto, isrDeduccion, imssDeduccion, neto };
}
...
// Se guardan bajo una carpeta por negocio (el mismo token de acceso de la URL) para no
// mezclar fotos entre distintos clientes que comparten este proyecto de Supabase.
const PHOTO_BUCKET = "fotos-checador";

function photoStoragePath(recordId) {
  const carpeta = ACCESO_TOKEN || "sin-negocio";
  return `${carpeta}/checador/${recordId}.jpg`;
}
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-4">
      ...
      <div className="rounded-sm p-4" style={{ background: paper }}>
        ...
        <button
          onClick={() => onGenerate(worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle)}
...
export default function RelojChecador() {
  ...
  // ---------- estado de cobro (regalo de bienvenida / bloqueo por falta de pago) ----------
  // null mientras carga. Cuando ya cargó: { bloqueado, contratoAceptado, requiereMetodoPago,
  // enPrueba, diasRestantes, diasPrueba, plan, precioMensual, nombreNegocio }.
  // "requiereMetodoPago" es nuevo (18 sep 2026): ya aceptó el contrato pero todavía no
  // registró su tarjeta en Stripe — es el paso que ahora va justo después del contrato,
  // antes de dejarlo usar la app (ver guardia de pantalla más abajo).
  const [estadoAcceso, setEstadoAcceso] = useState(null);

  async function recargarEstadoAcceso() {
    try {
      const { data, error } = await supabase.rpc("estado_acceso_reloj", { p_acceso_token: ACCESO_TOKEN });
      if (error) throw error;
      const fila = data && data.length ? data[0] : null;
      if (!fila || !fila.existe) {
      ...
      setEstadoAcceso({
        ...
        contratoAceptado: !!fila.contrato_aceptado,
        requiereMetodoPago: !!fila.requiere_metodo_pago,
        enPrueba: !!fila.en_prueba,
        diasRestantes: fila.dias_restantes,
        diasPrueba: fila.dias_prueba,
        plan: fila.plan,
        precioMensual: fila.precio_mensual,
        nombreNegocio: fila.nombre_negocio,
      });
      // El resto de la app sigue usando "tokenValido" para saber si puede mostrar algo.
      // Si está bloqueado por falta de pago, el link SÍ es válido — mostramos la
      // pantalla de cobro en vez de "Link no válido" (ver más abajo, cerca del render).
      setTokenValido(true);
    } catch {
      setTokenValido(false);
      setEstadoAcceso(null);
  ...
  // Si la persona ya inició sesión con su correo (o acaba de tocar el enlace que le
  // llegó), esto la desbloquea sola, sin pedirle nombre+clave. Convive con el sistema
  // de PIN de siempre: si nunca agregó su correo, esto simplemente no hace nada.
  useEffect(() => {
    if (tokenValido !== true) return;
    let activo = true;
    async function intentarDesbloqueoPorCorreo() {
      try {
        const { data: sesion } = await supabase.auth.getSession();
        if (!sesion?.session) return;
        const fila = await miSesionPorToken();
        if (!activo) return;
        if (fila) {
          sesionViaCorreo = true;
          verifiedUsuarioRef.current = fila.nombre;
          verifiedPinRef.current = "";
          setRolActivo(fila.rol);
          setUnlockedSession(true);
          setToast({ color: sage, text: `Hola, ${fila.nombre}. Entraste con tu correo.` });
        }
      } catch {
        // sin conexión o sesión inválida: se queda como estaba (puede seguir usando su PIN)
      }
    }
    intentarDesbloqueoPorCorreo();
    const { data: sub } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session) intentarDesbloqueoPorCorreo();
    });
    return () => {
      activo = false;
  ...
  // ---------- pago / suscripción de Stripe ----------
  // "activarPago" se usa en dos pantallas: la de "registra tu método de pago"
  // (justo después de aceptar el contrato — regalo de bienvenida de 10 días,
  // ver guardias de render más abajo) y la de bloqueo (si la suscripción se
  // canceló por falta de pago). "abrirPortalSuscripcion" se usa desde la
  // tarjeta de Suscripción y pago (Nómina, solo Dueño) — todas llaman a Edge
  // Functions de Supabase, nunca a Stripe directo desde el navegador. La Edge
  // Function crear-checkout-session ya arma la suscripción con 10 días de
  // regalo (trial_period_days) y pide la tarjeta desde el Checkout aunque no
  // se cobre nada todavía (payment_method_collection: "always").
  ...
  async function activarPago() {
    ...
    setPagoError("");
    try {
      const { data, error } = await supabase.functions.invoke("crear-checkout-session", {
        body: { acceso_token: ACCESO_TOKEN },
      });
      if (error) throw error;
      if (data?.url) {
      ...
      setPagoError("No se pudo iniciar el pago. Intenta de nuevo en un momento.");
      setPagoBusy(false);
    }
  }
  ...
  async function abrirPortalSuscripcion() {
    ...
    setPagoError("");
    try {
      const { data, error } = await supabase.functions.invoke("crear-portal-session", {
        body: { acceso_token: ACCESO_TOKEN },
      });
      if (error) throw error;
      if (data?.url) {
        ...
        throw new Error(data?.error || "No se pudo abrir el portal de suscripción.");
      }
    } catch (err) {
      setPagoError("No se pudo abrir tu suscripción. Intenta de nuevo en un momento.");
      setPagoBusy(false);
    }
  }
  ...
  function handleGeneratePayroll(worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle) {
  ...
  async function confirmAndGeneratePayroll() {
    ...
    const { employee: emp, worked, totals, draft, propinaSeSuma, vacacionesDetalle, feriadosDetalle } = confirmGeneratePayroll;
    ...
    const run = {
      ...
      // quién generó este recibo (Dueño o Gerente con sesión desbloqueada) — base para
      // el historial de nómina de la pestaña Control.
      registradoPor: verifiedUsuarioRef.current || "",
      registradoRol: rolActivo || "",
    };
    ...
    setPayrollPrintView({ employee: emp, draft: draft, worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle });
  ...
  async function saveFeriadosCatalogo(nuevoCatalogo) {
    let ok = false;
    try {
      ok = await guardarDatoProtegido(
        "feriados_catalogo",
        JSON.stringify(nuevoCatalogo),
  ...
  async function aplicarCorreccionManual() {
    ...
    const logEntry = {
      ...
      creadoEn: new Date().toISOString(),
      // quién hizo este registro (Dueño o Gerente con sesión desbloqueada) — no es un
      // campo editable, se toma directo de la sesión verificada contra Supabase.
      registradoPor: verifiedUsuarioRef.current || "",
      registradoRol: rolActivo || "",
    };
  ...
  async function guardarActa() {
    ...
    const nueva = {
      ...
      creadoEn: new Date().toISOString(),
      // quién elaboró el acta desde la sesión verificada (Dueño o Gerente) — distinto de
      // "elaboroNombre", que es el campo libre del documento legal.
      registradoPor: verifiedUsuarioRef.current || "",
      registradoRol: rolActivo || "",
    };
  ...
  // ---------- aceptación de contrato / aviso de privacidad (una vez por negocio) ----------
  const CONTRATO_VERSION_ACTUAL = "2026-09-18";
  const [contratoUI, setContratoUI] = useState({ nombre: "", checked: false, busy: false, error: "" });

  async function aceptarContrato() {
    const nombre = (contratoUI.nombre || "").trim();
    if (!nombre) {
      setContratoUI((s) => ({ ...s, error: "Escribe el nombre de quien acepta." }));
      return;
    }
    if (!contratoUI.checked) {
      setContratoUI((s) => ({ ...s, error: "Debes marcar que aceptas el Contrato y el Aviso de Privacidad." }));
      return;
    }
    setContratoUI((s) => ({ ...s, busy: true, error: "" }));
    try {
      const { data, error } = await supabase.rpc("cliente_aceptar_contrato_reloj", {
        p_acceso_token: ACCESO_TOKEN,
        p_nombre: nombre,
        p_version: CONTRATO_VERSION_ACTUAL,
      });
      if (error) throw error;
      if (!data) throw new Error("no_aceptado");
      await recargarEstadoAcceso();
      setContratoUI({ nombre: "", checked: false, busy: false, error: "" });
    } catch {
      setContratoUI((s) => ({ ...s, busy: false, error: "No se pudo registrar la aceptación. Intenta de nuevo." }));
    }
  }
  ...
  async function enviarMiEnlaceDeAcceso() {
    const correo = (pinModal.correo || "").trim();
    if (!correo || !correo.includes("@")) {
      setPinModal((m) => ({ ...m, correoMsg: "Escribe un correo válido." }));
      return;
    }
    setPinModal((m) => ({ ...m, correoBusy: true, correoMsg: "" }));
    try {
      await enviarEnlaceAcceso(correo);
      setPinModal((m) =>
        m ? { ...m, correoBusy: false, correoMsg: "Listo. Revisa tu correo y toca el enlace desde este mismo dispositivo." } : m
      );
    } catch {
      setPinModal((m) => (m ? { ...m, correoBusy: false, correoMsg: "No se pudo enviar el enlace. Revisa tu conexión." } : m));
    }
  }
  ...
  function openChangePin() {
    setPinModal({
      mode: "change",
      target: null,
      nombre: verifiedUsuarioRef.current,
      value: "",
      confirmValue: "",
      oldValue: "",
      error: "",
  ...
  async function guardarMiCorreo() {
    const correo = (pinModal.correo || "").trim();
    if (!correo || !correo.includes("@")) {
      setPinModal((m) => ({ ...m, correoMsg: "Escribe un correo válido." }));
      return;
    ...
    setPinModal((m) => ({ ...m, correoBusy: true, correoMsg: "" }));
    try {
      const ok = await asignarMiCorreo(verifiedUsuarioRef.current, verifiedPinRef.current, correo);
      setPinModal((m) =>
        m
          ? {
              ...m,
              correoBusy: false,
              correoMsg: ok
                ? "Listo. Desde ahora puedes entrar con un enlace a ese correo, sin escribir tu clave."
                : "No se pudo guardar (revisa tu conexión o que tu sesión de clave siga desbloqueada).",
            }
          : m
      );
    } catch {
      setPinModal((m) => (m ? { ...m, correoBusy: false, correoMsg: "No se pudo guardar. Revisa tu conexión." } : m));
    }
  }

  function lockNow() {
    if (sesionViaCorreo) {
      sesionViaCorreo = false;
  ...
  async function submitPin() {
    ...
    if (pinModal.mode === "unlock") {
      ...
      sesionViaCorreo = false; // entró con nombre+clave: manda eso, aunque haya una sesión de correo abierta de otra persona
  ...
  async function handleFile(e) {
    ...
    if (!file) return;
    try {
      // hora real del servidor si hay internet; si no, cae a la del celular pero marcada
      // como "verified: false" — la foto no se bloquea, solo queda avisada (ver más abajo
      // en confirmPunch, donde esta bandera se guarda en el registro para la Bitácora).
      const { date: stampDate, verified } = await getServerNow();
      const dataUrl = await compressImage(file, 480, 0.6, { watermark: true, stampDate, verified });
      setPunchModal((prev) => (prev ? { ...prev, photo: dataUrl, photoHoraVerificada: verified } : prev));
    } catch {
  ...
  async function confirmPunch() {
    ...
    try {
      // hora real del servidor si hay internet; si no hay conexión NO se bloquea el
      // checado (un corte de internet en una hora crítica no debe frenar puntualidad,
      // nómina ni propinas) — se usa la hora del dispositivo y se marca el registro
      // como "horaSinVerificar" para que se vea un aviso en Bitácora y el negocio
      // decida qué hacer con esos casos puntuales.
      const { date: now, verified: horaVerificada } = await getServerNow();
      ...
      const record = {
        ...
        // true si no se pudo confirmar la hora contra el servidor (se usó la del
        // dispositivo como respaldo) — se avisa en Bitácora para que el negocio decida.
        horaSinVerificar: !horaVerificada,
      };
      ...
      if (!horaVerificada) {
        // aviso inmediato de que se guardó igual, pero con hora sin confirmar — el
        // detalle completo queda visible después en la tarjeta del registro en Bitácora.
        toastText += " · ⚠ hora sin verificar con el servidor";
      }
      setToast({ color: horaVerificada ? (punctuality ? meta.color : sage) : paprika, text: toastText });
      setPunchModal(null);
  ...
  // ---------- guardia legal: aceptar Contrato de Servicio y Aviso de Privacidad (una vez por negocio) ----------
  // Va primero: nada de la app (ni siquiera la pantalla de pago) se muestra hasta aceptar.
  if (estadoAcceso && !estadoAcceso.contratoAceptado) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center p-6" style={{ background: charcoal }}>
        <div className="w-full max-w-sm rounded-sm p-6" style={{ background: paper }}>
          <div className="flex items-center gap-2 mb-2">
            <FileSignature size={20} color={brass} />
            <div className="text-sm font-bold" style={{ color: ink }}>Antes de empezar</div>
          </div>
          <p className="text-xs mb-3" style={{ color: ink + "99" }}>
            Para usar Reloj Checador, la persona responsable del negocio debe aceptar el Contrato de
            Servicio y el Aviso de Privacidad. Después de esto te pediremos registrar tu método de
            pago para activar tu regalo de bienvenida — no se te cobra nada durante esos días.
          </p>
          <div
            className="rounded-sm p-3 mb-3 text-xs overflow-y-auto"
            style={{ background: charcoal + "0d", color: ink + "aa", maxHeight: 160, border: `1px solid ${ink}22` }}
          >
            <p className="mb-2">
              <b>[BORRADOR — pendiente de revisión legal]</b> Al aceptar, confirmas que usarás Reloj Checador
              para registrar entradas, salidas, incidencias y propinas de tu personal, que darás de alta
              únicamente información veraz, y que informarás a tu personal que sus datos (nombre, horarios,
              fotos de check-in) se usan con este fin, conforme al Aviso de Privacidad de la app.
            </p>
            <p>
              Reloj Checador es una herramienta de apoyo administrativo; el negocio que la usa es responsable
              de cumplir con la legislación laboral y de protección de datos aplicable a su personal.
            </p>
          </div>
          <input
            type="text"
            placeholder="Nombre de quien acepta (dueño/gerente)"
            className="w-full mb-2 px-3 py-2 rounded-sm text-sm outline-none"
            style={{ border: `1px solid ${ink}33`, color: ink }}
            value={contratoUI.nombre}
            onChange={(e) => setContratoUI((s) => ({ ...s, nombre: e.target.value, error: "" }))}
            disabled={contratoUI.busy}
          />
          <label className="flex items-start gap-2 mb-3 text-xs" style={{ color: ink }}>
            <input
              type="checkbox"
              checked={contratoUI.checked}
              onChange={(e) => setContratoUI((s) => ({ ...s, checked: e.target.checked, error: "" }))}
              disabled={contratoUI.busy}
              className="mt-0.5"
            />
            <span>Acepto el Contrato de Servicio y el Aviso de Privacidad de Reloj Checador.</span>
          </label>
          {contratoUI.error && (
            <p className="text-xs mb-2" style={{ color: paprika }}>{contratoUI.error}</p>
          )}
          <button
            onClick={aceptarContrato}
            disabled={contratoUI.busy}
            className="w-full py-2 rounded-sm text-sm font-bold uppercase"
            style={{ background: brass, color: ink, opacity: contratoUI.busy ? 0.6 : 1 }}
          >
            {contratoUI.busy ? "Guardando…" : "Aceptar y continuar"}
          </button>
        </div>
      </div>
    );
  ...
  // ---------- guardia de pago: registrar método de pago (regalo de bienvenida de 10 días) ----------
  // Nueva (18 sep 2026): va justo después del contrato, antes de dejar usar la app. Ya no se
  // pide la tarjeta hasta que se acaba el plazo — se captura y se valida desde el inicio, pero
  // no se cobra nada hasta que pasan los días de regalo (lo maneja Stripe con trial_period_days,
  // ver crear-checkout-session). Si el negocio cancela antes de que se cumplan, nunca se le cobra.
  if (estadoAcceso && estadoAcceso.requiereMetodoPago) {
    const dias = estadoAcceso.diasPrueba ?? 10;
    return (
      <div className="min-h-screen w-full flex items-center justify-center p-6" style={{ background: charcoal }}>
        <div className="w-full max-w-sm rounded-sm p-6 text-center" style={{ background: paper }}>
          <Gift size={22} color={brass} className="mx-auto mb-2" />
          <div className="text-sm font-bold mb-1" style={{ color: ink }}>Tu regalo de bienvenida</div>
          <p className="text-xs mb-3" style={{ color: ink + "99" }}>
            {estadoAcceso.nombreNegocio ? `${estadoAcceso.nombreNegocio}: ` : ""}
            te regalamos <strong>{dias} días</strong> para usar {estadoAcceso.plan ? `el plan ${estadoAcceso.plan}` : "Reloj Checador"}{" "}
            y decidir si te conviene. Para activarlo necesitamos registrar tu método de pago —
            no se te cobra nada hoy.
          </p>
          {estadoAcceso.precioMensual != null && (
            <p className="text-xs font-bold mb-3" style={{ color: ink }}>
              Después de tus {dias} días de regalo: ${estadoAcceso.precioMensual} MXN / mes + IVA
            </p>
          )}
          <button
            onClick={activarPago}
            ...
            style={{ background: brass, color: ink }}
          >
            {pagoBusy ? <Loader2 size={15} className="animate-spin" /> : <Wallet size={15} />}
            {pagoBusy ? "Abriendo registro de pago…" : "Registrar método de pago"}
          </button>
          {pagoError && (
            <p className="text-[10px] mt-2" style={{ color: paprika }}>
              {pagoError}
            </p>
          )}
          <p className="text-[10px] mt-3" style={{ color: ink + "66" }}>
            Se abre la página segura de pago de Stripe. No se hace ningún cargo durante tus {dias} días
            de regalo — si cancelas antes de que terminen, no se te cobra nada. Si no cancelas, al
            terminar el plazo se cobra automáticamente el primer mes.
          </p>
        </div>
      </div>
    );
  ...
  // ---------- guardia de cobro: suscripción cancelada o sin pago activo ----------
  if (estadoAcceso && estadoAcceso.bloqueado) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center p-6" style={{ background: charcoal }}>
        <div className="w-full max-w-sm rounded-sm p-6 text-center" style={{ background: paper }}>
          <AlertTriangle size={22} color={paprika} className="mx-auto mb-2" />
          <div className="text-sm font-bold mb-1" style={{ color: ink }}>Sin suscripción activa</div>
          <p className="text-xs mb-3" style={{ color: ink + "99" }}>
            {estadoAcceso.nombreNegocio ? `${estadoAcceso.nombreNegocio}: ` : ""}
            No pudimos cobrar o tu suscripción {estadoAcceso.plan ? `del plan ${estadoAcceso.plan}` : ""} ya no
            está activa. Registra tu pago de nuevo para seguir usando la app.
          </p>
          {estadoAcceso.precioMensual != null && (
            <p className="text-xs font-bold mb-3" style={{ color: ink }}>
              ${estadoAcceso.precioMensual} MXN / mes + IVA
            </p>
          )}
          <button
            onClick={activarPago}
          ...
          >
            {pagoBusy ? <Loader2 size={15} className="animate-spin" /> : <Wallet size={15} />}
            {pagoBusy ? "Abriendo pago…" : "Activar pago"}
          </button>
          {pagoError && (
            <p className="text-[10px] mt-2" style={{ color: paprika }}>
              {pagoError}
          ...
          <p className="text-[10px] mt-3" style={{ color: ink + "66" }}>
            Se abre la página segura de pago de Stripe. Si prefieres, también puedes contactar a
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-6 pb-4" style={{ borderBottom: `2px dashed ${steel}55` }}>
        ...
        {estadoAcceso && estadoAcceso.enPrueba && (
          <div
            className="flex items-center gap-1.5 mt-2 px-2.5 py-1.5 rounded-sm"
            style={{ background: brass + "22", border: `1px solid ${brass}55` }}
          >
            <Gift size={13} color={brass} />
            <span className="text-[10px]" style={{ color: paper }}>
              {estadoAcceso.diasRestantes != null && estadoAcceso.diasRestantes > 0
                ? `Regalo de bienvenida: te quedan ${estadoAcceso.diasRestantes} día${estadoAcceso.diasRestantes === 1 ? "" : "s"}.`
                : "Tu regalo de bienvenida termina hoy."}
            </span>
          </div>
        )}
                      ...
                      {dayRecords.map((r) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {r.horaSinVerificar && (
                              <span
                                className="flex-shrink-0 flex items-center gap-1 text-[9px] font-bold uppercase px-1.5 py-1 rounded-sm text-right"
                                style={{ background: paprika + "22", color: paprika }}
                                title="Esta hora no se pudo confirmar contra el servidor (sin internet en ese momento) — se guardó con la hora del dispositivo. Revísala si es importante para nómina o propinas."
                              >
                                <AlertTriangle size={11} /> Sin verificar
                              </span>
                            )}
                            {meta?.color && (
                              <span
        ...
        <div className="flex-1 px-5 py-5 flex flex-col min-h-0 overflow-y-auto">
          {!payrollSelectedEmployeeId ? (
            <>
              ...
              {/* Suscripción y pago (Stripe) — visible para quien desbloqueó Nómina; el botón
                  para gestionar la suscripción (Customer Portal de Stripe) solo se muestra al
                  Dueño, igual que "Configuración del negocio y plan" en el resto de la app. */}
              <div className="rounded-sm p-4" style={{ background: paper }}>
                ...
                <div className="text-xs" style={{ color: ink }}>
                  <div>
                    Plan: <strong>{estadoAcceso?.plan || "—"}</strong>
                    {estadoAcceso?.precioMensual != null && ` · $${estadoAcceso.precioMensual} MXN/mes + IVA`}
                  </div>
                  <div className="mt-1" style={{ color: ink + "88" }}>
                    Estado:{" "}
                    {estadoAcceso?.enPrueba
                      ? `en tu regalo de bienvenida (${estadoAcceso.diasRestantes ?? 0} día(s) restantes)`
                      : estadoAcceso?.bloqueado
                      ? "sin suscripción activa"
                      : "activa"}
                  </div>
                </div>
                {rolActivo === "dueno" && !estadoAcceso?.enPrueba && !estadoAcceso?.bloqueado && (
                  <button
                    onClick={abrirPortalSuscripcion}
                    disabled={pagoBusy}
                    className="mt-3 flex items-center gap-1.5 text-[10px] font-bold uppercase px-2.5 py-1.5 rounded-sm disabled:opacity-60"
                    style={{ border: `1px solid ${ink}33`, color: ink }}
                  >
                    {pagoBusy ? <Loader2 size={12} className="animate-spin" /> : <Wallet size={12} />}
                    Gestionar mi suscripción
                  </button>
                )}
                {rolActivo === "dueno" && estadoAcceso?.bloqueado && !estadoAcceso?.enPrueba && (
                  <button
                    onClick={activarPago}
                    disabled={pagoBusy}
                    className="mt-3 flex items-center gap-1.5 text-[10px] font-bold uppercase px-2.5 py-1.5 rounded-sm disabled:opacity-60"
          ...
          <div className="w-full max-w-sm rounded-sm p-5" style={{ background: paper }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs font-bold uppercase" style={{ color: ink, letterSpacing: "0.06em" }}>
                  {pinModal.mode === "unlock" && (pinModal.viaCorreo ? "Entrar con tu correo" : "Ingresa la clave")}
            ...
            {pinModal.mode === "unlock" && pinModal.viaCorreo ? (
              <>
                <p className="text-xs mb-3" style={{ color: ink + "aa" }}>
                  Solo si ya agregaste tu correo antes (desde "Cambiar mi clave"). Te llega un enlace
                  para entrar sin escribir tu clave.
                </p>
                <input
                  autoFocus
                  type="email"
                  autoCapitalize="none"
                  value={pinModal.correo}
                  onChange={(e) => setPinModal((m) => ({ ...m, correo: e.target.value, correoMsg: "" }))}
                  onKeyDown={(e) => e.key === "Enter" && enviarMiEnlaceDeAcceso()}
                  placeholder="Tu correo"
                  disabled={pinModal.correoBusy}
                  className="w-full px-3 py-2.5 rounded-sm text-sm outline-none mb-2"
                  style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
                />
                {pinModal.correoMsg && (
                  <p className="text-xs mb-2" style={{ color: pinModal.correoMsg.startsWith("Listo") ? sage : paprika }}>
                    {pinModal.correoMsg}
                  </p>
                )}
              </>
            ...
            {pinModal.mode === "unlock" && (
              <button
                type="button"
                onClick={() =>
                  setPinModal((m) => ({ ...m, viaCorreo: !m.viaCorreo, error: "", correoMsg: "" }))
                }
                className="text-xs underline mb-3"
                style={{ color: ink + "99" }}
              >
                {pinModal.viaCorreo ? "← Usar nombre y clave" : "¿Prefieres entrar con tu correo?"}
              </button>
            )}
            ...
            {pinModal.mode === "change" && (
              <div className="mt-4 pt-4" style={{ borderTop: `1px solid ${ink}1a` }}>
                <p className="text-xs font-bold uppercase mb-1" style={{ color: ink + "aa", letterSpacing: "0.06em" }}>
                  Entrar sin clave (opcional)
                </p>
                <p className="text-xs mb-2" style={{ color: ink + "aa" }}>
                  Agrega tu correo y la próxima vez podrás entrar con un enlace, sin escribir tu clave.
                </p>
                <input
                  type="email"
                  autoCapitalize="none"
                ...
                <button
                  type="button"
                  onClick={guardarMiCorreo}
                  disabled={pinModal.correoBusy}
                  className="w-full py-2 rounded-sm font-bold text-xs uppercase"
                  style={{ border: `1px solid ${ink}33`, color: ink, opacity: pinModal.correoBusy ? 0.6 : 1 }}
                >
                  {pinModal.correoBusy ? "Guardando…" : "Guardar correo"}
                </button>
              </div>
            )}

            {pinModal.error && (
              <p className="text-xs mb-2 mt-3" style={{ color: paprika }}>
                {pinModal.error}
              </p>
            )}
            ...
            {pinModal.mode === "unlock" && pinModal.viaCorreo ? (
              <button
                onClick={enviarMiEnlaceDeAcceso}
                disabled={pinModal.correoBusy}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-sm font-bold text-sm uppercase mt-2"
                style={{ background: brass, color: ink, opacity: pinModal.correoBusy ? 0.6 : 1 }}
              >
                <LockOpen size={15} />
                {pinModal.correoBusy ? "Enviando…" : "Enviar enlace"}
              </button>
            ) : (
              <button
                onClick={submitPin}
