...
function computePayrollTotals(draft, worked, totalEligibleDiasPropina) {
  const base = draft.rateType === "hora" ? worked.totalHoras * draft.rateAmount : worked.totalDias * draft.rateAmount;
  const bono = draft.enableBono ? worked.diasBono * draft.bonoPorJornada : 0;
  const propina =
    draft.enablePropina && totalEligibleDiasPropina > 0
      ? (worked.diasPropina / totalEligibleDiasPropina) * (Number(draft.propinaPool) || 0)
      : 0;
  const vacaciones = draft.enableVacaciones ? (Number(draft.vacacionesDias) || 0) * draft.rateAmount : 0;
...
function PayrollPanel({
  ...
  const totalEligibleDiasPropina = draft.enablePropina
    ? computeTotalEligibleDiasPropina(draft.periodStart, draft.periodEnd, employees, recordsByDate)
    : 0;
  const totals = computePayrollTotals(draft, worked, totalEligibleDiasPropina);
      ...
      <div className="rounded-sm p-4 flex flex-col gap-2" style={{ background: paper }}>
        ...
        {draft.enablePropina && (
          <div className="pl-1">
            <div className="flex items-center gap-2">
              <span className="text-[11px]" style={{ color: ink + "88" }}>
                Bolsa total de propinas del periodo:
              </span>
              <input
                value={draft.propinaPool}
                onChange={(e) => onChange({ propinaPool: e.target.value.replace(/[^0-9.]/g, "") })}
                inputMode="decimal"
                className="flex-1 px-2 py-1.5 rounded-sm text-xs outline-none"
                style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
              />
            ...
            <p className="text-[10px] mt-1" style={{ color: ink + "66" }}>
              Se reparte entre todo el personal según sus días elegibles ({totalEligibleDiasPropina} días
              elegibles en total este periodo, de los cuales {worked.diasPropina} son de {employee.name}).
            </p>
          </div>
        )}
        ...
        {draft.enableVacaciones && (
          <div className="flex items-center gap-2 pl-1">
            ...
            <input
              value={draft.vacacionesDias}
              onChange={(e) => onChange({ vacacionesDias: e.target.value.replace(/[^0-9.]/g, "") })}
              inputMode="decimal"
              className="flex-1 px-2 py-1.5 rounded-sm text-xs outline-none"
              style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
            />
...
export default function RelojChecador() {
  ...
  function savePayrollConfig() {
    ...
    setToast({ color: sage, text: "Configuración de pago guardada." });
  }
    ...
    return (
      <div style={{ background: "#fff", minHeight: "100vh", color: ink, fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
        ...
        <div style={{ padding: "2rem", maxWidth: "560px", margin: "0 auto" }}>
          <h1 style={{ fontSize: "1.1rem", fontWeight: 900, textTransform: "uppercase" }}>Recibo de nómina</h1>
          <p style={{ fontSize: "0.8rem", color: ink + "99", marginTop: "2px" }}>
            Restaurante Bondiola · Periodo {periodLabel}
          </p>

          <div style={{ marginTop: "1rem", fontSize: "0.85rem", lineHeight: 1.6 }}>
            <div>
              <strong>Empleado:</strong> {pEmp.name}
            </div>
            <div>
              <strong>Puesto:</strong> {pEmp.puesto}
            </div>
            <div>
              <strong>Tarifa:</strong> {formatMoney(pDraft.rateAmount)} por {pDraft.rateType === "hora" ? "hora" : "jornada"}
            </div>
            <div>
              <strong>{pDraft.rateType === "hora" ? "Horas trabajadas" : "Días trabajados"}:</strong>{" "}
              {pDraft.rateType === "hora" ? pWorked.totalHoras : pWorked.totalDias}
            </div>
          </div>

          <table style={{ width: "100%", marginTop: "1.2rem", fontSize: "0.8rem", borderCollapse: "collapse" }}>
            <tbody>
              <tr style={{ borderBottom: `1px solid ${ink}15` }}>
                <td style={{ padding: "4px 0" }}>Pago base</td>
                <td style={{ padding: "4px 0", textAlign: "right" }}>{formatMoney(pTotals.base)}</td>
              </tr>
              {pDraft.enableBono && (
                <tr style={{ borderBottom: `1px solid ${ink}15` }}>
                  <td style={{ padding: "4px 0" }}>Bono por puntualidad</td>
                  <td style={{ padding: "4px 0", textAlign: "right" }}>+{formatMoney(pTotals.bono)}</td>
                </tr>
              )}
              {pDraft.enablePropina && (
                <tr style={{ borderBottom: `1px solid ${ink}15` }}>
                  <td style={{ padding: "4px 0" }}>Reparto de propina</td>
                  <td style={{ padding: "4px 0", textAlign: "right" }}>+{formatMoney(pTotals.propina)}</td>
                </tr>
              )}
              {pDraft.enableVacaciones && (
                <tr style={{ borderBottom: `1px solid ${ink}15` }}>
                  <td style={{ padding: "4px 0" }}>Vacaciones ({pDraft.vacacionesDias} días)</td>
                  <td style={{ padding: "4px 0", textAlign: "right" }}>+{formatMoney(pTotals.vacaciones)}</td>
                </tr>
              )}
              <tr style={{ borderBottom: `2px solid ${ink}33`, fontWeight: 700 }}>
                <td style={{ padding: "6px 0" }}>Bruto</td>
                <td style={{ padding: "6px 0", textAlign: "right" }}>{formatMoney(pTotals.bruto)}</td>
              </tr>
              {pDraft.enableLey && (
                <tr style={{ borderBottom: `1px solid ${ink}15`, color: paprika }}>
                  <td style={{ padding: "4px 0" }}>Descuento de ley ({pDraft.leyPercent}%)</td>
                  <td style={{ padding: "4px 0", textAlign: "right" }}>-{formatMoney(pTotals.leyDeduccion)}</td>
                </tr>
              )}
              {pDraft.enableDescuentos &&
                (pDraft.descuentos || []).map((d, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${ink}15`, color: paprika }}>
                    <td style={{ padding: "4px 0" }}>{d.label || "Descuento/penalización"}</td>
                    <td style={{ padding: "4px 0", textAlign: "right" }}>-{formatMoney(d.amount)}</td>
                  </tr>
                ))}
              {pDraft.enableConsumo &&
                (pDraft.consumo || []).map((c, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${ink}15`, color: paprika }}>
                    <td style={{ padding: "4px 0" }}>{c.label || "Consumo en el local"}</td>
                    <td style={{ padding: "4px 0", textAlign: "right" }}>-{formatMoney(c.amount)}</td>
                  </tr>
                ))}
              <tr>
                <td style={{ padding: "10px 0", fontWeight: 900, fontSize: "1rem" }}>Neto a pagar</td>
                <td style={{ padding: "10px 0", textAlign: "right", fontWeight: 900, fontSize: "1rem", color: sage }}>
          ...
          {pDraft.enableLey && (
            <p style={{ marginTop: "0.5rem", fontSize: "0.68rem", color: ink + "88" }}>
